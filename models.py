...
class DEN(object):
    def __init__(self, config):
        ...
        for i in range(self.n_layers - 1):
            w = self.create_variable('layer%d' % (i + 1), 'weight',
                                     [self.dims[i], self.dims[i + 1]])
            b = self.create_variable('layer%d' % (i + 1), 'biases',
                                     [self.dims[i + 1]])
    ...
    def extend_top(self, scope, ex_k=10):
        ...
        if 'layer%d' % self.n_layers == scope:
            ...
            expanded_n_units = self.expansion_layer[self.n_layers - level -
                                                    2]  # top-down
        ...
        if 'layer%d' % self.n_layers == scope:
            for i in self.task_indices:
                if i == self.T:  # current task(fragile)
                    ...
                    b = self.get_variable(scope, 'biases_%d' % i, True)
                    new_w = tf.get_variable(
                        'new_fc/n%d' % i, [ex_k, self.n_classes],
                        trainable=True)
                else:
                    # previous tasks
                    w = self.get_variable(scope, 'weight_%d' % i, False)
            ...
            new_w = self.create_variable(scope + '/new_fc', 'bw',
                                         [prev_dim, ex_k])
            ...
            new_w2 = self.create_variable(scope + '/new_fc', 'tw',
                                          [ex_k, next_dim + ex_k])
    ...
    def selective_learning(self, task_id, selected_params):
        ...
        with tf.variable_scope('layer%d' % self.n_layers):
            w = tf.get_variable(
                'weight_%d' % task_id,
                initializer=selected_params['layer%d/weight_%d:0' %
                                            (self.n_layers, task_id)])
            b = tf.get_variable(
                'biases_%d' % task_id,
                initializer=selected_params['layer%d/biases_%d:0' %
                                            (self.n_layers, task_id)])
            ...
            for var in GL_var:
                ...
                for i in range(var.get_shape()[1]):
                    ...
                    gw_gl = tf.where(
                        ...
                        tf.zeros(tf.shape(var[:, i])), temp_gw)
                    gw.append(gw_gl)
    ...
    def add_task(self, task_id, data):
        ...
        if self.T == 1:
            ...
            repeated, c_loss = self.run_epoch(self.opt, self.loss, trainX,
                                              trainY, 'Train')
            expansion_layer = [0, 0]
        else:
            """ SELECTIVE LEARN """
            ...
            for i in range(self.n_layers, 0, -1):
                if i == self.n_layers:
                    ...
                    selected_params['layer%d/weight_%d:0' % (
                        i,
                        task_id)] = w[np.ix_(all_indices['layer%d' % i], [0])]
                    ...
                    sub_weight = w[np.ix_(all_indices['layer%d' % i],
                                          top_indices)]
            ...
            _vars = [(var.name, self.sess.run(var))
            ...
            for i in range(self.n_layers, 0, -1):
                if i == self.n_layers:
                    ...
                    params['layer%d/biases_%d:0' %
                           (i,
                            task_id)] = selected_params['layer%d/biases_%d:0' %
                                                        (i, task_id)]
                else:
                    ...
                    temp_biases[all_indices['layer%d' % (
            ...
            if c_loss < self.loss_thr:
                ...
                for i in range(self.n_layers - 1, 0, -1):
                    ...
                    useless = []
                    for j in range(prev_layer_weight.shape[1] - self.ex_k,
                    ...
                    if i == self.n_layers - 1:
                        for t in self.task_indices:
                            prev_layer_weight = params['layer%d/weight_%d:0' %
                                                       (i + 1, t)]
                            cur_layer_weight = np.delete(
                                prev_layer_weight, useless, axis=0)
                            params['layer%d/weight_%d:0' %
                                   (i + 1, t)] = cur_layer_weight
                    else:
                        prev_layer_weight = params['layer%d/weight:0' %
                                                   (i + 1)]
                        cur_layer_weight = np.delete(
                            prev_layer_weight, useless, axis=0)
        ...
        test_preds, test_loss = self.sess.run(
            [self.yhat, self.loss], feed_dict={self.X: testX,
                                               self.Y: testY})
    ...
    def predict_perform(self, task_id, X, Y, task_name=None):
        ...
        print(" [*] Evaluation, Task:%s, test_acc: %.4f" % (str(task_name),
                                                            test_perf))
        return test_perf
